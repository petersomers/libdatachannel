...
/// Nalu fragment header (Primarily H.264 Fragmentation Unit A)
...
// VP8 and AV1 use an in-payload descriptor, so the base NAL unit size is 0 bytes of header.
static const size_t VP8_NAL_HEADER_SIZE  = 0;
static const size_t AV1_NAL_HEADER_SIZE  = 0; // AV1 uses OBU descriptor in payload
...
struct RTC_CPP_EXPORT NalUnit : binary {
    ...
    // ADDED AV1
    enum class Type { H264, H265, VP8, AV1 };

    NalUnit(const NalUnit &unit) = default;
    
    // Helper to determine base header size based on type
    constexpr size_t getHeaderSize(Type type) const {
        if (type == Type::H264) return H264_NAL_HEADER_SIZE;
        if (type == Type::H265) return H265_NAL_HEADER_SIZE;
        // VP8 and AV1 are treated as having no common prefix header here
        return 0; 
    }
    ...
    NalUnit(Type type = Type::H264)
        : binary(getHeaderSize(type)) {}

    template <typename Iterator> NalUnit(Iterator begin_, Iterator end_) : binary(begin_, end_) {}

    // WARNING: The following methods are only valid for H.264/H.265 type NAL units
    ...
    // WARNING: Only valid for H.264/H.265 type NAL units
    binary payload() const {
        // Assume H.264/H.265 if size() >= 1, otherwise assume VP8/AV1 or error.
        size_t hSize = size() >= H264_NAL_HEADER_SIZE ? H264_NAL_HEADER_SIZE : 0; 
        if (hSize > 0) {
            return {begin() + hSize, end()};
        }
        // For VP8/AV1, the whole buffer is payload/descriptor
        return {begin(), end()};
    }

    void setForbiddenBit(bool isSet) { 
        if (size() >= 1) header()->setForbiddenBit(isSet); 
    }
    void setNRI(uint8_t nri) { 
        if (size() >= 1) header()->setNRI(nri); 
    }
    void setUnitType(uint8_t type) { 
        if (size() >= 1) header()->setUnitType(type); 
    }

    void setPayload(binary payload) {
        // This method is now ambiguous for VP8/AV1 which have no fixed header.
        // Keeping it for H.264/H.265 compatibility.
        size_t hSize = size() >= H264_NAL_HEADER_SIZE ? H264_NAL_HEADER_SIZE : 0;
        assert(size() >= hSize);
        erase(begin() + hSize, end());
...
/// Nal unit fragment A (H.264/H.265 specific)
